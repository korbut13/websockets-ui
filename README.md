## Description 
 Websocket battleship server is implemented using websocket.
...
## Game description
1. Inmemory DB with player data (login and password) storage
...
- Game
    - Attack\
        ...
        ```
    - Attack feedback (it's sent after every shot, miss and after kill sent miss for all cells around ship too)\    
        ```->```
        ```ts
        {
            type: "attack";,
