...
export const attackHandler = (req: Request) => {
  ...
  if (nextPlayer.index === indexPlayer) {
    ...
    const g = dataBaseGames.get(gameId)?.get(indexPlayer)?.moves.find((move) => move.x === xPosition && move.y === yPosition);
    if (g === undefined) {
      dataBaseGames.get(gameId)?.get(indexPlayer)?.moves.push({ x: xPosition, y: yPosition })
      ...
      console.log("There has already been such a move")
    }
  } else {
